...
export async function GET(request: NextRequest) {
  ...
  if (!color) {
    return Response.json({ error: 'Color parameter is required' }, { status: 400 });
  }

  // Use mock data for now while database is being set up
  const filteredThemes = mockThemes.filter(theme => 
    theme.primary_color.toLowerCase().includes(color.toLowerCase()) ||
    theme.secondary_color?.toLowerCase().includes(color.toLowerCase()) ||
    theme.accent_color?.toLowerCase().includes(color.toLowerCase())
  ).sort((a, b) => b.popularity_score - a.popularity_score);

  return Response.json(filteredThemes.slice(0, 50));
  
  /* Database code (commented out for now)
  const supabase = createClient();
  
  try {
    // Search for themes that contain this color in any of the color fields
    const { data, error } = await supabase
      .from('theme_collections')
      .select('*')
      .or(`primary_color.ilike.%${color}%,secondary_color.ilike.%${color}%,accent_color.ilike.%${color}%`)
      .order('popularity_score', { ascending: false })
      .limit(50);

    if (error) {
      console.error('Error searching themes by color:', error);
